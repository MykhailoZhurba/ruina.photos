/**
 * Interface text in every language.
 *
 * English is the source: its keys define what every other language must
 * provide, and TypeScript rejects a dictionary with a missing or extra key.
 * `{name}` placeholders are filled in by `t()`; a test checks every language
 * keeps the same placeholders as English.
 *
 * Page-length prose (the About bio, the privacy notice) lives in src/content as
 * one Markdown file per language instead.
 */
...
const en = {
	...
	'booking.intro':
		'Leave your email and I will write back with the details. Everything below the email is optional — fill in what you already know.',
	'booking.email': 'Email',
	'booking.name': 'Name',
...
const lv: Dictionary = {
	...
	'about.photoAlt':
		'Mykhailo Zhurba ar teleobjektīvu lidlaukā, aiz viņa uz skrejceļa — vieglā lidmašīna',
	'about.email': 'E-pasts',
	...
	'booking.intro':
		'Atstājiet savu e-pastu, un es atbildēšu ar sīkāku informāciju. Pārējie lauki nav obligāti — aizpildiet to, ko jau zināt.',
	...
	'booking.ok.stored':
		'Paldies! Jūsu pieteikums ir saņemts, un es drīz sazināšos ar jums pa e-pastu.',
	...
	'booking.err.rate_limited':
		'Pārāk daudz pieteikumu pēc kārtas — lūdzu, mēģiniet vēlreiz nedaudz vēlāk.',
	'booking.err.server_error': 'Radās kļūda manā pusē. Lūdzu, mēģiniet vēlreiz.',
	'booking.err.network':
		'Neizdevās sazināties ar serveri. Lūdzu, pārbaudiet interneta savienojumu.',
...
const ru: Dictionary = {
	...
	'about.photoAlt':
		'Mykhailo Zhurba с телеобъективом на аэродроме, позади на взлётной полосе — лёгкий самолёт',
	...
	'booking.intro':
		'Оставьте свой e-mail, и я напишу вам с подробностями. Всё, что ниже, заполнять необязательно — укажите то, что уже знаете.',
	...
	'booking.ok.stored':
		'Спасибо! Ваша заявка получена, скоро я свяжусь с вами по электронной почте.',
...
const uk: Dictionary = {
	...
	'about.photoAlt':
		'Mykhailo Zhurba з телеоб’єктивом на аеродромі, позаду на злітній смузі — легкий літак',
	...
	'booking.intro':
		'Залиште свій e-mail, і я напишу вам із подробицями. Усе, що нижче, заповнювати необов’язково — вкажіть те, що вже знаєте.',
	'booking.email': 'E-mail',
	'booking.name': 'Ім’я',
	...
	'booking.ok.stored':
		'Дякую! Вашу заявку отримано, незабаром я зв’яжуся з вами електронною поштою.',
...
export const ui: Record<Locale, Dictionary> = { en, lv, ru, uk };
...
/** Returns `t(key, vars)` for one language. */
export function useTranslations(locale: Locale) {
	const dict = ui[locale] ?? ui.en;
	const t = (key: UiKey, vars: Vars = {}): string => fill(dict[key] ?? ui.en[key], vars);
	/** For keys built at runtime (e.g. from a collection id): falls back when missing. */
	const tOr = (key: string, fallback: string, vars: Vars = {}): string =>
		key in dict ? fill(dict[key as UiKey], vars) : fallback;
	return { t, tOr };
}

/** `Something else` → `shoot.something_else`, the key of a shoot type's label. */
export function shootTypeKey(value: string): string {
