/**
 * Completes Google sign-in: verifies the state, exchanges the code, checks the
 * account against the allowlist, and stores the session plus the refresh token
 * the Calendar integration runs on.
 */
...
/** A plain, self-contained page; the admin layout is not available pre-session. */
...
export const GET: APIRoute = async ({ url, cookies, locals, redirect }) => {
	...
	if (code === null || returnedState === null || expectedState === undefined) {
	...
	if (!isAllowedAdmin(email, env.ADMIN_EMAILS ?? '')) {
		// Deliberately explicit: this is the owner's own dashboard, and a wrong
		// Google account is the likeliest reason to land here.
		return message(
			'Not authorised',
